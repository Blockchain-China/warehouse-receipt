...
func main() {
    ...
    config.ClientID = "Ouyeel_WarehouseReceipt_Blockchain"
    config.Producer.Return.Successes = true
    config.Producer.Partitioner = sarama.NewRandomPartitioner    
    producer, err := sarama.NewSyncProducer([]string{"0.0.0.0:9092"}, config)
    if err != nil {
        fmt.Printf("Failed to create produce: %s\n", err)
        os.Exit(-1)
    }
    defer func() {
	...
	for {
		...
		case ce := <-a.cEvent:
			...
			fmt.Printf("Chaincode TxID:%s\n", ce.ChaincodeEvent.TxID)
			//ChaincodeEvent.EventName=>receiptId
			fmt.Printf("Chaincode EventName:%s\n", ce.ChaincodeEvent.EventName)
			//ChaincodeEvent.Payload=>receiptJson
			fmt.Printf("Chaincode EventPayload:%s\n", string(ce.ChaincodeEvent.Payload))

			msg := &sarama.ProducerMessage{}
            msg.Topic = "oyqkl" 
            msg.Key = sarama.StringEncoder(ce.ChaincodeEvent.EventName)
            msg.Value = sarama.ByteEncoder(string(ce.ChaincodeEvent.Payload))
